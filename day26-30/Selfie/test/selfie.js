...
describe('[Challenge] Selfie', function () {
	...
	it('Execution', async function () {
		...
		const setActionTx = await attackerContract.connect(player).setAction();
		await setActionTx.wait();

		// Advance time 2 days so that action can get executed
		await ethers.provider.send('evm_increaseTime', [2 * 24 * 60 * 60]); // 2 days

		const attackTx = await attackerContract.connect(player).attack();
		await attackTx.wait();
